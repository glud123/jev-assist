---
name: jev-assist
description: "Whole-repo judgments a linter cannot make, across a pool too large to read. Load this skill BEFORE searching, on the shape of the question alone: \"which files still do X\", \"what hasn't moved to Y yet\", \"where would this change land\", \"is this diff safe to commit\". Then grep, and let the probe's outcome route you — often to neither command, which is a decision this skill makes with you rather than instead of you. Do not defer the load until after the search: by then the per-file judgment is already hand-coded as a pile of grep exclusions, and that is how this goes unused. `rerank` ranks every file against a task when the task's words and the code's don't overlap. `drift` asks one question of every file when a literal floods but cannot separate real instances from noise. `gate` judges a staged diff for auth, PII, data-loss and swallowed-error risk. `validate` measures rerank's recall here. Nothing to install: one `node` call on a script path."
---

...
jev-assist 把这类判断交给 [TypeSafe Jev](https://docs.typesafe.ai)——它回答封闭问题，返回的是带概率
的类型化值而非散文。三个命令由此而来：`rerank` 按任务相关度给全部文件排名，`drift` 用团队约定逐个检
查文件，`gate` 判断待提交的 diff 有没有碰到危险的地方。
...
**`jev rerank "<任务>"`** 拿一句话的任务描述给每个被跟踪的文件排名。在一个 705 文件的私有 React 应
用上重放 10 个真实提交，前 20 条覆盖了这些提交实际改动文件的 68%，前 40 条覆盖 80%。平均值是最没意
思的那部分：凡是只碰了 1 到 4 个文件的提交都是满分，碰了 11 个文件的那个只捞回 5 个。所以前 20 条要
当成一次改动的核心来读，但不要当成全部——截断线之后名次已经不可靠，与其继续往下翻，不如顺着头部结果
的 import 找。

它赢过 grep 的地方是那些压根没提这个功能的文件。一个共享的上传服务，位于应用里完全不相干的另一个角
落，在一个图片上传任务里排到第 6。搜 `avatar` 或者 `profile` 的人是找不到它的。

**`jev drift [glob]`** 拿你的约定去问你的文件，每个文件一次调用。问题数量几乎不影响成本，所以一次
问十件事而不是一件：没翻译的文案、绕开了请求封装的代码、活在 store 之外的服务端状态。和 grep 的区别
在于输入：grep 要你先把违规写成一个能匹配的模式，而这里你写下的是约定本身，哪些代码违反它交给模型
判断。

**`jev gate [ref]`** 读已暂存的 diff 并标出风险，命中即非零退出，所以 pre-commit 钩子里只需要这一
条命令。在四个真实提交上测过：一个解密功能 0.99，一个修补被吞掉错误的改动 0.91，一个 CSS 微调
0.64，一个只改注释的 0.03。每个 diff 0.7 到 1.8 秒。类型检查、lint 和单元测试都会放过一个悄悄丢掉
用户输入的改动，这条命令存在的全部理由就在这儿。

**`jev validate [n]`** 最该先跑，也最没人跑。它把你自己的提交当任务重放，用每次提交真正改了什么来给
排名打分。准确率不能跨仓库转移，所以上面那些数字只是我们的。你的 git 历史就是免费的标注数据。
...
`rerank` 只打印前 N 条。确实需要读到截断线之外时加 `--json`，它会额外把完整排名写进仓库根目录的
`.jev-rerank.json`，记得把这个文件加进 `.gitignore`。

打印出来的列表要看分数的断崖，不要按行数读。`topN` 是固定条数，不是相关度筛选——一个实际只牵涉 3 个
文件的任务同样会打印 20 行，后面那 17 行只是本次排名里最不无关的文件而已。分数是四个档位上的概率分
布，从「没理由为这个任务打开它」到「很可能得读或改」，所以 0.3 的意思是背景文件，不是第 4 名。
...
放到一个没有 i18n 的应用里字面上就不成立，只会把每个文件都标红。
...
| `validateK` | 否 | `[20, 40]` | `validate` 计算召回率的截断线 |

`conventions` 和 `gates` 里每一项的子字段：

| 子字段 | 说明 |
| --- | --- |
| `ask` | 向模型提的封闭问题。锚定到具体模块（"`src/services` 里的那个请求封装"），不要锚定到抽象原则 |
...
一条约定去问一个并不存在的请求封装，check 会放过，然后把每个文件都标红。所以让 agent 说清楚哪些约定
...
够 n 个碰了 `include` 范围内文件的提交，用每条标题对整个仓库排名，然后打印各截断线上的召回率。
...
- **rerank 看不见结构性耦合。** 一个仅仅因为它 import 的东西变了而被改动的文件没有语义信号，这样的
  文件有一个落在了 705 里的第 121 位。顺着头部结果的 import 往下找。

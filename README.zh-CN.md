...
现有做法各有其边界。grep 要求任务的用词在代码里出现，词对不上——任务换了语言、文案只存在 i18n key
里、代码对功能的叫法不同——它就两手空空；让模型通读全仓库，则要付出与文件数成正比的时间和 token。
两者都不适合「对每个文件问同一个问题」这类需要规模化的判断。
...
skill 的作用是让 agent 在该用的时候自己用。你不需要记命令——把任务像对同事那样说出来：自己的话、自己的
语言，不用给文件名，前提是会话的工作目录就是那个要被判断的仓库。
...
```
用户注销后，其定时导出任务还在跑——找出是哪里没处理
```

skill 的描述写明了 agent 什么时候该自己触发，而决定权始终在 grep 这一把探针。探到收敛在一处，这里
什么都不跑——agent 直接读文件。否则由探针的结果挑命令：

- **落空**，或者泛洪到没法**排名** → `rerank`。任务用词和代码用词对不上：任务换了语言、文案只存在
  i18n key 里、代码对同一功能的叫法不同。
- **泛洪**到没法**分类** → `drift`。字面量能匹配上，但它答的不是你问的问题，要把真正的实例挑出来，
  得先知道这个文件是什么、这处匹配意味着什么——模式带不了这种上下文。
- **待提交的 diff** → `gate`。

排名还是分类，是泛洪该走哪条路的分水岭：*这些文件里哪些和我要改的东西有关*要的是一个次序，*这些命中
里哪些是真的*要的是每个文件一个是非。措辞本身从不触发任何东西——问得再泛，只要用词能和代码对上，那
还是 grep 的活。

点名 skill 也可以，在它没自己触发时兜底：
...
agent 会按 `SKILL.md` 里的说明调 `scripts/jev.mjs`，把排名和命中项读给你听。第一次在新仓库上用，
让它先跑 `jev validate 20`——排名准不准是一个可以测出来的事实，不是需要相信的说法。
...
上重放 10 个真实提交：前 20 条覆盖实际改动文件的 68%，前 40 条覆盖 80%。对比 grep 的优势在于命中与任务
不共享任何词的文件——图片上传任务中，与应用其他部分毫无关联的共享上传服务排到第 6。打印条数之外
